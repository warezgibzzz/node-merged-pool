                    ...
                    GetBlockTemplate(function() {
                        if (indexes.length > 0){
                            for(var i = 0;i < indexes.length;i++) {
                                emitLog('Updating work for auxillary chains: ' + options.auxes[i].name);
                            }
                        } else {
                            emitLog('Updating work for auxillary chain: ' + options.auxes[0].name);
                        }
                    }, true);
                }
            finishedCallback();
            ...
            var auxResult = function(hash, aux) {
                ...
                CheckBlockAccepted(hash, _this.auxes[aux].daemon, function(accepted, tx, height, value) {
                    ...
                    _this.auxes[aux].daemon.cmd('gettransaction', [tx], function(res) {
						...
						_this.emit('blockinfo', options.auxes[aux], height, hash,tx, cmdResponse.details[0].amount, shareData);
                        }
                    });
                    UpdateAuxes();
            ...
            if (!isValidBlock)
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, _this.daemon, function(isAccepted, tx, height, value){
                        isValidBlock = true;  // blockHash may be PoWHash so this may fail. Accept it and check later.
                        shareData.txHash = tx;
                        emitShare();
						...
						_this.emit('blockinfo', options.coin, shareData.height, shareData.blockHash, tx, shareData.blockReward * 0.00000001, shareData);
